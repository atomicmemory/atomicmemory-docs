...
in the same shape. There is no "internal" vs "external" observability
split.
...
```json
{
  ...
  "observability": {
    "retrieval": {
      "candidateIds": ["m-a", "m-b", "m-c", "m-d", "m-e"],
      "candidateCount": 5,
      "queryText": "what did we decide about retries?",
      "skipRepair": false
    },
    "packaging": {
      "packageType": "subject-pack",
      "includedIds": ["m-a", "m-b", "m-c"],
      "droppedIds": ["m-d", "m-e"],
      "evidenceRoles": { "m-a": "primary", "m-b": "supporting", "m-c": "contextual" },
      "episodeCount": 2,
      "dateCount": 1,
    ...
    "assembly": {
      "finalIds": ["m-a", "m-b", "m-c"],
      "finalTokenCost": 184,
      "tokenBudget": null,
      "primaryEvidencePosition": 1,
      "blocks": ["subject"]
    }
  }
}
...
- **Regression tests.** Assert that for a known query, `candidateIds`
  includes the memory you expect and `primaryEvidencePosition === 1`.
  If packaging drifts, the test fails before prompt quality does.
- **Eval dashboards.** Aggregate `tokenCost`, `droppedIds` rates, and
...
- **Budget tuning.** `finalTokenCost` vs `tokenBudget` across real
