...
`createCoreRuntime` is the composition root. It takes an explicit dependency bundle — today, just the Postgres pool — and returns a fully wired `CoreRuntime` containing config, repos, stores, and services:
